...
function PostsPanel(){
  ...
  async function load(){ const r = await fetch(`${API_BASE}/api/posts`); setPosts(await r.json()) }
  async function save(){ if(!editing) return; await fetch(`${API_BASE}/api/meta`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ filename: editing, title, tagline }) }); setEditing(null); load() }

  useEffect(()=>{ load() }, [])

  return (
    <div className="bg-white border border-[#dcdcdc] rounded p-4 space-y-2">
      <div className="flex items-center justify-between">
        <h3 className="font-sans font-semibold">Posts</h3>
      ...
      <ul className="divide-y divide-[#eee]">
        {posts.map(p => (
          <li key={p.filename} className="py-3 flex items-center justify-between">
      ...
      {editing && (
        <div className="border-t border-[#ccc] pt-3">
          ...
          <button onClick={save} className="px-4 py-2 bg-[#052962] text-white rounded text-sm">Save</button>
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-[#f6f6f6] text-[#121212]">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        {show && (<>
          <Uploader token={token} onUploaded={()=>{}} />
          <ShortsTab token={token} />
          <PostsPanel />
