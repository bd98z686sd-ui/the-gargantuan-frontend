...
export default function ShortsTab({ token, toast }){
  ...
  const [maxSeconds, setMax] = useState(45)
  ...
  async function enqueue(){
    if(!sel) return toast?.show('Pick audio','error')
    const r = await fetch(`${API_BASE}/api/shorts/request`, {
      method:'POST', headers:{'Content-Type':'application/json','x-admin-token':token},
      body: JSON.stringify({ filename: sel, maxSeconds })
    })
  ...
  return (
    <div className="bg-white border border-[#dcdcdc] rounded p-4 space-y-3">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-center">
        <select value={sel} onChange={e=>setSel(e.target.value)} className="border border-[#dcdcdc] rounded px-3 py-2 text-sm flex-1">
          ...
          {aud.map(a => <option key={a.filename} value={a.filename}>{a.title||a.filename}</option>)}
        </select>
      ...
      <div className="pt-2">
        <h4 className="font-sans font-semibold mb-2">Completed</h4>
