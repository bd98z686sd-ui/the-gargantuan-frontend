...
export default function Trash({ token, toast }){
  ...
  async function load(){
    ...
    try{
      ...
      setItems(data);
      setSelected({});
    }catch(e){ toast?.show('Failed to load trash','error'); }
  ...
  async function restoreOne(filename){
  ...
  async function hardDeleteOne(filename){
      ...
      {!loading && rows.length>0 && (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((r)=> <Row key={r.key} row={r} onRestore={restoreOne} onHardDelete={hardDeleteOne} onToggle={toggleOne} checked={!!selected[r.id]} />)}
            </tbody>
          </table>
        </div>
      )}
    ...
    <tr className="border-t border-[#eee]">
      ...
      <td className="py-2">
        <div className="flex gap-2">
          <button onClick={()=>onRestore(row.filename)} className="px-3 py-1 bg-[#052962] text-white rounded">Restore</button>
          <a href={row.url} target="_blank" className="px-3 py-1 border border-[#dcdcdc] rounded">Open</a>
          <button onClick={()=>onHardDelete(row.filename)} className="px-3 py-1 bg-[#c70000] text-white rounded">Delete permanently</button>
        </div>
      </td>
    </tr>
  );
